...
class MoneyTransferViewModel(val applicationScope: CoroutineScope) : ViewModel() {
        ...
        job = viewModelScope.launch {
            withContext(Dispatchers.Default) {
                try {
                    ...
                    // Ensure active before checking funds
                    ensureActive()
                    ...
                    // Ensure active before start debit
                    ensureActive()
                    debitAccount(state.savingsBalance, amountToTransfer)
                    // Ensure the transfer is finished even if job is cancelled
